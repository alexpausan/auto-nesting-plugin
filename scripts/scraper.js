...
;(async () => {
  ...
  function getChildrenWithoutExtraDivs(node) {
    ...
    if (
      children.length === 1 &&
      CONTAINER_TAGS[nodeName] &&
      CONTAINER_TAGS[children[0]?.nodeName]
    ) {
  ...
  function getOrientationBasedOnRects(props) {
    ...
    const horDiff =
      horizontalPosOfCenter[horizontalPosOfCenter.length - 1] - horizontalPosOfCenter[0]
